...
describe('beautiful.ai Test POC', () => {
  ...
  context('Create presentations from prompts', () => {
      it('should generate slide based on pre-defined prompt', () => {
        // select prompt
        homePage.elements.pre_gen_prompt('Who is Yoda?').click()
        // verify prompt text and slide text
      ...
      it('should generate slide based on user prompt', () => {
        ...
        homePage.elements.prompt().type('Top 5 Marvel movies ranked by worldwide gross')
        homePage.elements.generateBtn().click()
        // Verify slide text
        cy.contains('div', 'Avengers: Endgame', { timeout: 10_000}).should('be.visible')
